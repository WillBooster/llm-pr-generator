...
# Output the description using GitHub Actions environment file
if [ -n "$GITHUB_OUTPUT" ]; then
  echo "pr-description<<EOF" >> $GITHUB_OUTPUT
  echo "$DESCRIPTION" >> $GITHUB_OUTPUT
  echo "EOF" >> $GITHUB_OUTPUT
  echo "Setting output using GITHUB_OUTPUT: $GITHUB_OUTPUT"
else
  # For environments where GITHUB_OUTPUT is not available
  echo "Warning: GITHUB_OUTPUT environment variable not set. Output may not be captured correctly."
fi
